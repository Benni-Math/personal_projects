...
unsafe fn advance(bodies: &mut [body; BODIES_COUNT]) {
    ...
    // This is how we wrap unsafe code
    // In this case, we are accessing a union (overlapping memory)
    impl Interactions {
        pub fn as_scalars(&mut self) 
            -> &mut [f64; ROUNDED_INTERACTIONS_COUNT] 
        {
            /// Safety: the in-memory representation of `f64` and `__m128d`
            /// is compatible, so access is safe
            /// (as long as `ROUNDED_INTERACTIONS_COUNT` is even)
            unsafe {
                &mut self.scalars
            }
        }
    ...
    for i in 0..ROUNDED_INTERACTIONS_COUNT/2 {
        ...
        for m in 0..3 {
            position_Delta[m] =
                position_Deltas[m].as_vectors()[i];
        }
