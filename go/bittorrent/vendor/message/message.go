...
func ParsePiece(index int, buf []byte, msg *Message) (int, error) {
    if msg.ID != MsgPiece {
        return 0, fmt.Errorf("Expected PIECE (ID %d), got ID %d)",
                                MsgPiece, msg.ID)
    }

    if len(msg.Payload) < 8 {
        return 0, fmtErrorf("Payload too short. %d < 8", len(msg.Payload))
    }
    ...
    if begin >= len(buf) {
        return 0, fmt.Errorf("Begin offset too high. %d >= %d",
                                begin,  len(buf))
    }

    data := msg.Payload[8:]
    if begin+len(data) > len(buf) {
        return 0, fmt.Errorf("Data too long [%d] for offset %d with length %d",
...
func ParseHave(msg *Message) (int, error) {
    if msg.ID != MsgHave {
        return 0, fmt.Errorf("Expected HAVE (ID %d), got ID %d",
                                MsgHave, msg.ID)
    }

    if len(msg.Payload) != 4 {
        return 0, fmt.Errorf("Expected payload length 4, got length %d",
                                len(msg.Payload))
    }
...
funx (m *Message) name() string {
    ...
    case MsgCancel:
        return "Cancel"
    default:
        return fmt.Sprintf("Uknown#%d", m.ID)
    }
}
